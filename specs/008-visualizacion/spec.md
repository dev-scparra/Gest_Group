...
| VIS-FR-009 (añadido por 014, **revisado por [015](../015-captura-guiada-combos/spec.md)**) | DEBE dibujar un **HUD de captura de combos** a partir de `estado_combo: EstadoCombo \| None` (del `CapturadorCombo`, 015). Cuando no es `None`, asiste al usuario según la fase: **INACTIVO** → pista breve ("haz un gesto para empezar"); **CAPTURANDO_G1/G2** → título de fase (gesto 1/2 de 2), gesto líder de la votación + su conteo, barra de progreso con la cuenta regresiva (en frames y segundos aproximados vía `fps`); **ESPERA** → "prepara el gesto 2" + `g1` capturado + barra; **RESULTADO** → `g1 o g2 = compuesto`. Debe usar un panel oscurecido (`cv2.addWeighted`) para legibilidad sobre cualquier fondo, y la flecha/símbolos que las fuentes Hershey soportan (nada de `≅`, ver VIS-FR-005). Reemplaza la línea simple de combo de 014 (`ResultadoCombinacion`). Valor pedagógico central: hace visible en pantalla que la composición `∘` del grupo se está aplicando en vivo, y guía al usuario por la captura. El HUD no decide nada (008 no calcula fase ni votos): solo renderiza el `EstadoCombo` que 015 ya computó. |
